...
mpirun --hostfile machinefile.$JOB_ID -np $process snappyHexMesh -overwrite -parallel  >& "$localDir"/simulation/log.snappyHexMesh
...
echo "SnappyHexMesh_Time: $execution_time" >> "$localDir"/simulation/log.time  # Write the execution time to the log.time file

mpirun --hostfile machinefile.$JOB_ID -np $process topoSet -parallel >& "$localDir"/simulation/log.topoSet

mpirun --hostfile machinefile.$JOB_ID -np $process createPatch -parallel -overwrite >& "$localDir"/simulation/log.createPatch
...
mpirun --hostfile machinefile.$JOB_ID -np $process patchSummary -parallel >& "$localDir"/simulation/log.patchSummary

mpirun --hostfile machinefile.$JOB_ID -np $process potentialFoam -parallel -writephi >& "$localDir"/simulation/log.potentialFoam

mpirun --hostfile machinefile.$JOB_ID -np $process checkMesh -parallel -writeFields '(nonOrthoAngle)' -constant >& "$localDir"/simulation/log.checkMesh

# Time simpleFoam and append the time to log.time
start_time=$(date +%s.%N)  # Get the start time in seconds with nanoseconds precision
mpirun --hostfile machinefile.$JOB_ID -np $process simpleFoam -parallel >& "$localDir"/simulation/log.simpleFoam
end_time=$(date +%s.%N)  # Get the end time in seconds with nanoseconds precision
execution_time=$(echo "$end_time - $start_time" | bc)  # Calculate the execution time
echo "SimpleFoam_Time: $execution_time" >> "$localDir"/simulation/log.time  # Write the execution time to the log.time file

reconstructParMesh -constant >& "$localDir"/simulation/log.reconstructParMesh

reconstructPar -latestTime >& "$localDir"/simulation/log.reconstructPar
